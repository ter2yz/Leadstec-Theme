...
<footer class="relative w-full py-10 bg-zinc-900 flex flex-col justify-center items-center z-40">
    ...
    <div class="relative container">
        <div class="relative flex flex-col lg:flex-row items-start lg:items-center justify-center lg:justify-between py-10 px-10 lg:px-20 bg-gradient-to-br from-[#F18701] to-[#FBB100] -mt-10 -translate-y-1/2 z-10 overflow-hidden">
            <h3 class="relative capitalize text-white text-[20px] font-semibold mb-6 lg:mb-0 z-10"><?php echo $banner_section['heading'] ?></h3>
            <a href="<?php echo $banner_section['button_url'] ?>" class="relative capitalize text-white flex justify-center items-center border-2 border-zinc-900 bg-zinc-900 px-6 py-2 z-10 whitespace-nowrap text-[14px]">
    ...
    <div class="container">
        <div class="w-full flex justify-start items-center py-6 text-zinc-300 text-3xl">
            <?php if(get_field('facebook_url', $footer_page->ID)): ?>
            <a href="<?php echo get_field('facebook_url', $footer_page->ID) ?>" class="mr-6 last:mr-0 transition opacity-50 hover:opacity-100">
                <i class="fa-brands fa-facebook"></i>
            </a>
            <?php endif; ?>
            <?php if(get_field('linkedin_url', $footer_page->ID)): ?>
            <a href="<?php echo get_field('linkedin_url', $footer_page->ID) ?>" class="mr-6 last:mr-0 transition opacity-50 hover:opacity-100">
                <i class="fa-brands fa-linkedin"></i>
            </a>
            <?php endif; ?>
            <?php if(get_field('weibo_url', $footer_page->ID)): ?>
            <a href="<?php echo get_field('weibo_url', $footer_page->ID) ?>" class="mr-6 last:mr-0 transition opacity-50 hover:opacity-100">
                <i class="fa-brands fa-weibo"></i>
            </a>
            <?php endif; ?>
            <?php if(get_field('youtube_url', $footer_page->ID)): ?>
            <a href="<?php echo get_field('youtube_url', $footer_page->ID) ?>" class="mr-6 last:mr-0 transition opacity-50 hover:opacity-100">
