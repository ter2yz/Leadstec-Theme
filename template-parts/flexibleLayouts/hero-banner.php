    ...
    <div class="w-[91%] max-w-none lg:max-w-[1112px] mx-auto z-10">
        <div class="w-full flex flex-col-reverse lg:flex-row justify-start lg:justify-center items-center lg:min-h-[600px] overflow-hidden">
            <div class="relative w-full flex flex-col items-center z-10 text-white">
                <?php if(get_sub_field('hero_section')['logo']):?>
                <img src="<?php echo esc_url(get_sub_field('hero_section')['logo']) ?>" alt="<?php echo get_sub_field('hero_section')['logo_image_alt_text'] ?>" class="w-20 mb-6">
                <?php endif; ?>
                <h1 class="text-[30px] md:text-[60px] font-bold leading-tight text-center max-w-6xl mb-5"><?php echo get_sub_field('hero_section')['heading'] ?></h1>
                <p class="leading-relaxed max-w-5xl text-center text-[15px] md:text-[20px]"><?php echo get_sub_field('hero_section')['description'] ?></p>
                <?php if(get_sub_field('hero_section')['button_url'] && get_sub_field('hero_section')['button_text']): ?>
                <a href="<?php echo esc_url(get_sub_field('hero_section')['button_url']) ?>" class="text-[14px] md:text-[18px] font-bold mt-5 flex justify-center items-center border-2 border-zinc-900 bg-zinc-900 px-6 py-2">
                    <?php echo get_sub_field('hero_section')['button_text'] ?>
                </a>
                <?php endif; ?>
            </div>
        </div>
