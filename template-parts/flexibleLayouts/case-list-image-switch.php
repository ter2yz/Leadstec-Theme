<section id="<?php echo get_sub_field('id')?>" class="case-list-image-switch w-full flex justify-center">
    <div class="w-[91%] max-w-none lg:max-w-[1112px] mx-auto">
        <div class="w-full flex flex-col lg:flex-row justify-between items-start lg:items-center px-2">
            <h2 class="text-[30px] md:text-[60px] text-[#1b1c1d] font-bold max-h-screen mb-0 w-full lg:w-auto md:text-left <?php echo get_sub_field('is_header_center') ? 'text-center' : 'text-left'; ?>"><?php echo get_sub_field('heading') ?></h2>
            <a href="<?php echo get_sub_field('read_more_button_url') ?>" class="hidden lg:flex text-[16px] font-bold flex-none justify-center items-center border-2 border-zinc-900 bg-zinc-900 px-6 py-2 text-white capitalize whitespace-nowrap">
        ...
        <div class="hidden lg:flex relative w-full h-full py-10 justify-center items-center px-2">
            <div class="relative flex flex-col items-center pr-6 w-1/5">
                ...
                <?php if( get_sub_field('cases')): ?>
                    <div class="slider-stories w-full">
                    <?php
                        $stories_index = 0;
                        foreach(get_sub_field('cases') as $story) {
                    ?>
                        ...
                        <div class="absolute inset-0 w-full h-full transition-all duration-500 opacity-0 hover:opacity-100 z-20">
                            <div class="description-wrap absolute w-1/2 h-full flex flex-col justify-start max-h-[67%] transition-all duration-500 bottom-0 right-0 bg-white p-6 text-zinc-900 overflow-auto z-20">
                                ...
                                <div class="absolute left-0 bottom-0 w-full bg-white px-6 py-3">
                                    <a href="<?php echo $story['button_url']; ?>" class="description-read-btn text-sky-600 text-left"><?php echo get_sub_field('read_more_button_label') ?></a>
                                </div>
                            </div>
                        </div>
                    </div>
